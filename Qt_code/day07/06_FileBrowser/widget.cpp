...
void Widget::setWidgetLayout(){
    ...
    QTreeWidgetItem *documents = new QTreeWidgetItem(dirTree,{"文档"});
    QTreeWidgetItem *desktop = new QTreeWidgetItem(dirTree,{"桌面"});
    QTreeWidgetItem *downloads = new QTreeWidgetItem(dirTree,{"下载"});
...
void Widget::setupConnections(){
    connect(dirTree, &QTreeWidget::itemClicked, this, &Widget::showDirectoryContents);
    connect(fileTable, &QTableWidget::itemDoubleClicked, this, &Widget::openFile);
    connect(dirTree, &QTreeWidget::customContextMenuRequested,
            this, &Widget::showTreeContextMenu);
    connect(fileTable, &QTableWidget::customContextMenuRequested,
            this, &Widget::showTableContextMenu);
}
...
void Widget::setupContextMenu(){
    ...
    // 点击"刷新"时调用函数
    connect(refreshAction, &QAction::triggered, this, &Widget::refreshTree);
    ...
    // 链接"打开"信号槽, 打开选中的文集N
    connect(openAction, &QAction::triggered, this, &Widget::openSelectedFile);
    // 链接"删除"信号槽,删除选中的文件
    connect(deleteAction, &QAction::triggered, this, &Widget::deleteSelectedFile);
}

void Widget::showDirectoryContents(QTreeWidgetItem *item, int){
    // 模拟显示目录内容
...
// 当用户点击我呢见列表的"打开"菜单项时,尝试打开当前选中的文件
void Widget::openSelectedFile(){
    // 获取当前选中的表格项
...
void Widget::deleteSelectedFile(){
    if(fileTable->currentItem()){
        // 获取当前选中行的索引
